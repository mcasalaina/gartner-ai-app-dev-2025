...
const menuNavigationTool = {
    type: "function",
    function: {
        name: "showMenuSection",
        description: "Navigate to and display a specific section or item on the restaurant menu",
        parameters: {
            type: "object",
            properties: {
                hashtag: {
                    type: "string",
                    description: "The hashtag/anchor ID to navigate to on the menu (e.g., 'steaks', 'filet-mignon', 'appetizers', etc.)"
                }
            },
            required: ["hashtag"]
        }
...
function showMenuSection(hashtag) {
    const menuIframe = document.querySelector('#menuSection iframe')
    if (menuIframe) {
        const currentSrc = menuIframe.src
        const baseSrc = currentSrc.split('#')[0] // Remove any existing hash
        menuIframe.src = `${baseSrc}#${hashtag}`
        return `Navigated to menu section: ${hashtag}`
    }
...
function handleUserQuery(userQuery, userQueryHTML, imgUrlPath) {
    ...
    let url = "{AOAIEndpoint}/openai/deployments/{AOAIDeployment}/chat/completions?api-version=2024-02-15-preview".replace("{AOAIEndpoint}", azureOpenAIEndpoint).replace("{AOAIDeployment}", azureOpenAIDeploymentName)
    let body = JSON.stringify({
        messages: messages,
        tools: [menuNavigationTool],
        tool_choice: "auto",
        stream: true
    })

    if (dataSources.length > 0) {
        url = "{AOAIEndpoint}/openai/deployments/{AOAIDeployment}/extensions/chat/completions?api-version=2024-02-15-preview".replace("{AOAIEndpoint}", azureOpenAIEndpoint).replace("{AOAIDeployment}", azureOpenAIDeploymentName)
        body = JSON.stringify({
    ...
    .then(response => {
        ...
        function read(previousChunkString = '') {
            return reader.read().then(({ value, done }) => {
                ...
                if (done) {
                    // Stream complete - handle any pending tool calls and add to messages
                    if (pendingToolCalls.length > 0) {
                        toolCallsExecuted = handleToolCalls(pendingToolCalls)
                        ...
                        if (toolCallsExecuted.length > 0) {
                            toolCallsExecuted.forEach(toolResult => {
                                messages.push({
                                    role: 'tool',
                            ...
                            // Make a follow-up call to get the assistant's response about what was shown
                            makeFollowUpCall()
                        }
                    } else {
                                ...
                                if (delta.tool_calls) {
                                    delta.tool_calls.forEach(toolCallDelta => {
                                        if (toolCallDelta.index !== undefined) {
                                            ...
                                            if (!pendingToolCalls[toolCallDelta.index]) {
                                                pendingToolCalls[toolCallDelta.index] = {
                                                    id: '',
                                                    type: 'function',
                                            ...
                                            const toolCall = pendingToolCalls[toolCallDelta.index]
    ...
    .then(() => {
        ...
        // Note: Assistant message and tool results are now added in the stream reading completion
    })
}

// Function to make a follow-up call after tool execution
function makeFollowUpCall() {
    const azureOpenAIEndpoint = document.getElementById('azureOpenAIEndpoint').value
    const azureOpenAIApiKey = document.getElementById('azureOpenAIApiKey').value
    const azureOpenAIDeploymentName = document.getElementById('azureOpenAIDeploymentName').value

    // Add a prompt asking the assistant to describe what was just shown
    messages.push({
        role: 'user',
        content: 'Please briefly describe, in no more than one short sentence, what you just showed me. Say something like "Here\'s the classic filet mignon."'
    })
    ...
    .then(response => {
        if (!response.ok) {
            throw new Error(`Follow-up API response status: ${response.status} ${response.statusText}`)
        }
                    ...
                    try {
                        if (line.startsWith('data:') && !line.endsWith('[DONE]')) {
                            ...
                            if (dataSources.length === 0) {
                                ...
                                if (role !== 'tool') {
                                    responseToken = responseJson.choices[0].messages[0].delta.content
                                    if (responseToken !== undefined && byodDocRegex.test(responseToken)) {
                                        responseToken = responseToken.replace(byodDocRegex, '').trim()
                                    }
                                    if (responseToken === '[DONE]') {
                                        responseToken = undefined
                        ...
                        console.log(`Error occurred while parsing follow-up response: ${error}`)
                    }
                })
    ...
    .catch(error => {
        console.log(`Error in follow-up call: ${error}`)
    })
}
...
function handleToolCalls(toolCalls) {
    ...
    toolCalls.forEach(toolCall => {
        if (toolCall.function.name === 'showMenuSection') {
            try {
                const args = JSON.parse(toolCall.function.arguments)
                const result = showMenuSection(args.hashtag)
                console.log('Menu navigation result:', result)
                ...
                toolResults.push({
                    id: toolCall.id,
                    result: 'Error navigating to menu section'
                })
            }
        }
    })
